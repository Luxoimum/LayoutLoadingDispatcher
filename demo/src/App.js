...
import { LayoutProvider, useLayout } from "@luxoimum/layout-loading-dispatcher";
...
function ExplanationOfThisDemo() {
  const [tick, setTick] = useState(0);
      ...
      <Grid item>
        ...
        <Paper className={classes.explanation} elevation={0}>
          ...
          <Typography
            className={classes.insideDoubleItem}
            variant={"body2"}
          >{`<ExplanationOfThisDemo />`}</Typography>
          <Typography
            className={classes.insideItem}
            variant={"body2"}
          >{`</LayoutProvider>`}</Typography>
